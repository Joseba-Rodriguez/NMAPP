...
def parse_for_individual(ip, hostname, portnum, protocol, service, versioning, vuln):
    ...
    cursor.execute("INSERT INTO nmapIndividual(ip, hostname, port, protocol,service, version, vuln) VALUES (%s, %s, %s, %s, %s, %s, %s)", (ip, hostname, portnum, protocol, service, versioning, vuln))
    conn.commit()

def raw_parser(NUM):
    """
    This function parses an NMAP XML file and inserts the data into a PostgreSQL database.
    :param NUM: an integer representing a parameter for the function. If it is 1 the code executes parse_for_individual. This is like this because in a future you can implement another function
    with this parser.
    """
    conn = psycopg2.connect(host=hostdb, database=database, user=user, password=password)
    cursor = conn.cursor()
    ...
    for host in root.findall('host'):
        ...
        for port in host.find('ports').findall('port'):
            ...
            vuln = port.find('script').get('output').replace("'", "") if port.find('script') is not None and port.find('script').get('output') is not None else ""
            product = port.find('service').get('product') if port.find('service') is not None and port.find('service').get('product') is not None else ""
            version = port.find('service').get('version') if port.find('service') is not None and port.find('service').get('version') is not None else ""
            extrainfo = port.find('service').get('extrainfo') if port.find('service') is not None and port.find('service').get('extrainfo') is not None else ""
            versioning = product.replace(",", "")
            versioning += ' (' + version + ')' if version else ''
            versioning += ' (' + extrainfo + ')' if extrainfo else ''
            versioning = product.replace("'", "")
            print(f"Dato insertado {ip}, {hostname}, {portnum}, {protocol}, {service}, {versioning}, {vuln}\n")
            if NUM != 1:
                parse_for_individual(ip, hostname, portnum, protocol, service, versioning, vuln)
NUM = int(sys.argv[1])
raw_parser(NUM)
